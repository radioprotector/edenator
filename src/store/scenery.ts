import { Vector2, Vector3, Euler, BufferGeometry, ConeGeometry, SphereGeometry, TorusGeometry, BoxGeometry, CylinderGeometry } from 'three';
...
export const SceneryMap: { [i in SceneryKey]: Scenery } = {
  [SceneryKey.ConePrimitive]: {
    primitive: new ConeGeometry(SCENERY_RADIUS, SCENERY_RADIUS * 3, undefined, undefined, true),
    primitiveDesc: 'cone'
  },
  [SceneryKey.SphereTopPrimitive]: {
    // / The top half of a sphere - make sure we move it down so it's flush
    primitive: new SphereGeometry(SCENERY_RADIUS, undefined, undefined, undefined, undefined, 0, Math.PI / 2),
    primitiveDesc: 'sphere half',
    translate: new Vector2(0, -SCENERY_RADIUS/2)
  },
  [SceneryKey.TorusTopPrimitive]: {
    ...
    primitive: new TorusGeometry(SCENERY_RADIUS, SCENERY_RADIUS / 2, undefined, undefined, Math.PI).rotateY(Math.PI / 2),
    primitiveDesc: 'torus half',
    translate: new Vector2(0, -SCENERY_RADIUS/1.5)
  },
  [SceneryKey.ColumnPrimitive]: {
    // A tall rectangular column
    primitive: new BoxGeometry(SCENERY_RADIUS, SCENERY_RADIUS * 5, SCENERY_RADIUS),
    primitiveDesc: 'rect column'
  },
  [SceneryKey.CylinderPrimitive]: {
    primitive: new CylinderGeometry(SCENERY_RADIUS, SCENERY_RADIUS, SCENERY_RADIUS * 5, undefined, undefined, true),
    primitiveDesc: 'cyl column'
  },
  // Cacti
  [SceneryKey.CactusShortModel]: {
    modelName: 'cactus_short',
    scale: new Vector3(SCENERY_RADIUS, SCENERY_RADIUS, SCENERY_RADIUS),
    translate: new Vector2(0, -SCENERY_RADIUS/2)
  },
  ...
  [SceneryKey.FlowerAModel]: {
    ...
    scale: new Vector3(SCENERY_RADIUS * 2, SCENERY_RADIUS * 2, SCENERY_RADIUS * 2),
    translate: new Vector2(0, -SCENERY_RADIUS/2)
  },
  ...
  [SceneryKey.PumpkinModel]: {
    ...
    scale: new Vector3(SCENERY_RADIUS * 1.5, SCENERY_RADIUS * 1.5, SCENERY_RADIUS * 1.5),
    translate: new Vector2(0, -SCENERY_RADIUS/2)
  },
  ...
  [SceneryKey.StoneTallGModel]: {
    ...
    scale: new Vector3(SCENERY_RADIUS, SCENERY_RADIUS, SCENERY_RADIUS),
    translate: new Vector2(0, -SCENERY_RADIUS/2),
    rotate: new Euler(0, Math.PI, 0) // Rotate 180 degrees to face the viewer
  },
